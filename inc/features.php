<?php
    if (!defined('ABSPATH')) {
        exit; // Exit if accessed directly
    } ?>
    <div class="abtb_doc_panel" data-tabs="#abptb_feature">
        <div class="abtb_doc_page_head">
            <h2>✨ Features</h2>
            <p class="abtb_doc_lede">
                ABP Transport Booking provides a powerful and flexible passenger transport booking system designed for
                managing transports, routes, schedules, tickets, seats, bookings, and passengers from a single
                platform. It includes the essential tools required to manage modern bus, ferry, launch, shuttle, coach,
                van, minibus, and other passenger transport services.
            </p>
        </div>

        <p class="abtb_doc_section_intro">
            The cards below group every feature of the plugin. Overarching capabilities such as the
            all-in-one transport booking dashboard and the transport management system apply across the whole plugin,
            while additional capabilities — marked <strong>PRO</strong> — are available in the paid version.
        </p>

        <div class="abtb_doc_cards">

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📊</span>
                    <h3>Core System</h3>
                </div>
                <p class="abtb_doc_card_lead">Top-level capabilities shared by the entire platform:</p>
                <ul>
                    <li>All-in-one transport booking dashboard</li>
                    <li>Transport management system</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🚌</span>
                    <h3>Transport Management</h3>
                </div>
                <ul>
                    <li>Create, edit, clone, view, and delete transports</li>
                    <li>Manage transport information and configurations</li>
                    <li>Transport list with pagination</li>
                    <li>Related transport display</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🛣️</span>
                    <h3>Route &amp; Schedule Management</h3>
                </div>
                <ul>
                    <li>Route management</li>
                    <li>Boarding and drop-off stop management</li>
                    <li>Multiple pickup points</li>
                    <li>Multiple drop-off points</li>
                    <li>Boarding and drop-off stop types</li>
                    <li>Route direction configuration</li>
                    <li>Date-wise schedules</li>
                    <li>Travel time configuration</li>
                    <li>Dynamic intermediate route allocation</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🔁</span>
                    <h3>One-Way &amp; Return Trip Booking</h3>
                </div>
                <ul>
                    <li>One-way trip booking</li>
                    <li>Return trip booking</li>
                    <li>Return booking using the same transport</li>
                    <li>Return route configuration</li>
                    <li>Return trip pricing and schedule configuration</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🎫</span>
                    <h3>Global Ticket Type Management</h3>
                </div>
                <ul>
                    <li>Create reusable ticket types</li>
                    <li>Assign ticket types to seat plans</li>
                    <li>Ticket name, color, icon/image, and prefix configuration</li>
                    <li>Reuse ticket types across multiple transports</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🪑</span>
                    <h3>Reusable Seat Plan System</h3>
                </div>
                <ul>
                    <li>Create reusable global seat plans</li>
                    <li>Assign seat plans to multiple transports</li>
                    <li>Reuse the same seat plan multiple times within one transport</li>
                    <li>Multi-layer seat plan support</li>
                    <li>Clone, edit, view, and delete seat plans</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🎨</span>
                    <h3>Advanced Seat Plan Designer</h3>
                </div>
                <ul>
                    <li>Drag-and-drop seat positioning</li>
                    <li>Automatic seat numbering</li>
                    <li>Custom seat labels and prefixes</li>
                    <li>Dynamic ticket type assignment</li>
                    <li>Custom seat cells and text</li>
                    <li>Flexible rows and columns</li>
                    <li>Adjustable cell dimensions</li>
                    <li>Adjustable spacing and gaps</li>
                    <li>Multiple layout configurations</li>
                    <li>Background and visual customization</li>
                    <li>Decor item support</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📡</span>
                    <h3>Real-Time Seat Reservation</h3>
                </div>
                <ul>
                    <li>Display available seats</li>
                    <li>Display booked and sold seat status</li>
                    <li>Interactive seat selection</li>
                    <li>Ticket-type based seating</li>
                    <li>Multi-layer seat availability</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">💰</span>
                    <h3>Flexible Pricing &amp; Quantity Control</h3>
                </div>
                <ul>
                    <li>Dynamic price calculation</li>
                    <li>Ticket and seat-based pricing</li>
                    <li>Minimum quantity configuration</li>
                    <li>Maximum quantity configuration</li>
                    <li>Reserved quantity support</li>
                    <li>Return trip pricing configuration</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🗓️</span>
                    <h3>Availability Management</h3>
                </div>
                <ul>
                    <li>Date-wise availability</li>
                    <li>Schedule-based availability</li>
                    <li>Weekend availability controls</li>
                    <li>Special date configuration</li>
                    <li>Off-date configuration</li>
                    <li>Availability override options</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">👤</span>
                    <h3>Passenger Information Forms</h3>
                </div>
                <ul>
                    <li>Global passenger forms</li>
                    <li>Transport-specific passenger forms</li>
                    <li>Custom passenger fields</li>
                    <li>Passenger information collection during booking</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🧰</span>
                    <h3>Additional Services</h3>
                </div>
                <ul>
                    <li>Global additional services</li>
                    <li>Transport-specific services</li>
                    <li>Optional service selection</li>
                    <li>Service pricing</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🌐</span>
                    <h3>Global Reusable Configuration</h3>
                </div>
                <p class="abtb_doc_card_lead">Reusable data types defined once and shared across transports:</p>
                <ul>
                    <li>Global ticket types</li>
                    <li>Global seat plans</li>
                    <li>Global stops</li>
                    <li>Global categories</li>
                    <li>Global organizers</li>
                    <li>Global brands</li>
                    <li>Global features</li>
                    <li>Global additional services</li>
                    <li>Global passenger forms</li>
                    <li>Global FAQs</li>
                    <li>Global Terms &amp; Conditions</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">⚙️</span>
                    <h3>Global &amp; Transport-Specific Configuration</h3>
                </div>
                <ul>
                    <li>Import global configuration</li>
                    <li>Customize imported configuration for individual transports</li>
                    <li>Global feature enable/disable controls</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📦</span>
                    <h3>Booking &amp; Order Management</h3>
                </div>
                <ul>
                    <li>Filter bookings and orders</li>
                    <li>Paginated booking/order list</li>
                    <li>Detailed booking information</li>
                    <li>Passenger information</li>
                    <li>Ticket and seat information</li>
                    <li>Order cancellation</li>
                    <li>WooCommerce order integration</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🌍</span>
                    <h3>Frontend Booking Experience</h3>
                </div>
                <ul>
                    <li>Transport listing pages</li>
                    <li>Search and filtering</li>
                    <li>Transport details</li>
                    <li>Route and stop information</li>
                    <li>Schedule information</li>
                    <li>Travel date selection</li>
                    <li>Return date selection</li>
                    <li>Ticket type selection</li>
                    <li>Seat selection</li>
                    <li>Passenger information</li>
                    <li>Additional service selection</li>
                    <li>Dynamic pricing</li>
                    <li>WooCommerce checkout</li>
                    <li>Responsive booking interface</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📋</span>
                    <h3>Customer My Account Page</h3>
                </div>
                <ul>
                    <li>&quot;Transport Bookings&quot; section inside the WooCommerce My Account page</li>
                    <li>Lists the logged-in customer's own bookings only</li>
                    <li>Paginated bookings list</li>
                    <li>Shows booking status and details</li>
                    <li>Login required to view bookings</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">❓</span>
                    <h3>FAQ &amp; Terms and Conditions</h3>
                </div>
                <ul>
                    <li>Global FAQ configuration</li>
                    <li>Transport-specific FAQ configuration</li>
                    <li>Global Terms &amp; Conditions</li>
                    <li>Transport-specific Terms &amp; Conditions</li>
                    <li>Import and customize global configuration</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🛒</span>
                    <h3>WooCommerce Integration</h3>
                </div>
                <ul>
                    <li>WooCommerce checkout integration</li>
                    <li>WooCommerce payment gateway support</li>
                    <li>WooCommerce order management</li>
                    <li>WooCommerce tax and coupon support</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🔗</span>
                    <h3>Transport Listings &amp; Shortcodes</h3>
                </div>
                <ul>
                    <li>Transport listing shortcode</li>
                    <li>Booking shortcode</li>
                    <li>Passenger PDF download shortcode (PRO)</li>
                    <li>Transport gallery shortcode</li>
                    <li>Search and filtering</li>
                    <li>Multiple layout styles</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📱</span>
                    <h3>Responsive &amp; Translation Ready</h3>
                </div>
                <ul>
                    <li>Fully responsive frontend interface</li>
                    <li>Translation-ready</li>
                </ul>
            </div>

            <div class="abtb_doc_card abtb_doc_card_wide">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">⭐</span>
                    <h3>PRO Features</h3>
                </div>
                <p class="abtb_doc_card_lead">The following capabilities are available in the PRO version:</p>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🛤️</span>
                    <h3>Multiple Trip Management</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <ul>
                    <li>Same-day multiple trips</li>
                    <li>Day-wise multiple trips</li>
                    <li>Date-wise multiple trips</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📄</span>
                    <h3>PDF Ticket &amp; Invoice</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📤</span>
                    <h3>Booking List Export</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <ul>
                    <li>PDF booking export</li>
                    <li>CSV booking export</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🔳</span>
                    <h3>QR Code Support</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">📧</span>
                    <h3>Email Notifications with PDF Attachments</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <ul>
                    <li>Email booking notifications</li>
                    <li>PDF ticket/invoice attachment</li>
                    <li>Resend email with PDF</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🏷️</span>
                    <h3>Advanced Pricing &amp; Discounts</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <ul>
                    <li>Seasonal pricing and discounts</li>
                    <li>Early booking discounts</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">💰</span>
                    <h3>Partial Payment</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <p class="abtb_doc_card_lead">Let customers pay a deposit now and settle the balance later:</p>
                <ul>
                    <li>Site-wide On/Off switch for the whole feature</li>
                    <li>Deposit as a fixed amount or a percentage of the total</li>
                    <li>Global default plus an independent per-transport override</li>
                    <li>Customer chooses <em>Full Amount</em> or <em>Deposit</em> at checkout</li>
                    <li><em>Balance Due Later</em> fee keeps the remaining amount visible on the order</li>
                    <li>Deposit paid and remaining balance stored on the order</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🗂️</span>
                    <h3>Advanced Booking Management</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">✏️</span>
                    <h3>Edit Existing Bookings</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <ul>
                    <li>Edit a booking directly from the Order List via the ✏️ button</li>
                    <li>Change transport, route, journey date and departure time</li>
                    <li>Adjust ticket quantities or rearrange the seat plan</li>
                    <li>Edit passenger details and additional services</li>
                    <li>Update billing information and order status</li>
                    <li>Live stock availability with own-booking stock excluded</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🧾</span>
                    <h3>My Account PDF Download &amp; Cancellation</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
                <p class="abtb_doc_card_lead">Customer-facing tools on the My Account bookings list:</p>
                <ul>
                    <li>Download the PDF ticket/invoice button for each booked booking</li>
                    <li>&quot;Request Cancellation&quot; button on a customer's booking</li>
                    <li>Request allowed only within the time limit set under Global Data → Cancel Request</li>
                    <li>&quot;Cancel Request Pending&quot; status badge while waiting</li>
                    <li>Admin approves or rejects requests from the Order List Cancel Requests tab</li>
                    <li>Approval cancels the booking and makes its seats/tickets available again</li>
                </ul>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🛠️</span>
                    <h3>Additional Administrative Tools</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">🎧</span>
                    <h3>Premium Support</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
            </div>

            <div class="abtb_doc_card">
                <div class="abtb_doc_card_head">
                    <span class="abtb_doc_card_icon">⚡</span>
                    <h3>Priority Updates &amp; New Features</h3>
                    <span class="abtb_doc_badge abtb_doc_badge_pro">PRO</span>
                </div>
            </div>

        </div>
    </div>
<?php